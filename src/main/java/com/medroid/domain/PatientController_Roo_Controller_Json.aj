...
privileged aspect PatientController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> PatientController.showJson(@PathVariable("id") Long id) {
    ...
    public ResponseEntity<String> PatientController.createFromJson(@RequestBody String json, UriComponentsBuilder uriBuilder) {
        HttpHeaders headers = new HttpHeaders();
        ...
        try {
            ...
            headers.add("Location",uriBuilder.path(a.value()[0]+"/"+patient.getId().toString()).build().toUriString());
    ...
    public ResponseEntity<String> PatientController.updateFromJson(@RequestBody String json, @PathVariable("id") Long id) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        try {
            Patient patient = Patient.fromJsonToPatient(json);
            patient.setId(id);
            if (patient.merge() == null) {
                return new ResponseEntity<String>(headers, HttpStatus.NOT_FOUND);
            }
    ...
    public ResponseEntity<String> PatientController.deleteFromJson(@PathVariable("id") Long id) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        try {
            Patient patient = Patient.findPatient(id);
